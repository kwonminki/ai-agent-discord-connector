...
- `/harness create goal:<goal>` opens a dedicated read-only Builder. The optional `source` choice either forks the exact current agent session or starts without its conversation history. The Builder runs a persisted discovery → design → review → ready interview instead of generating files immediately.
- `/harness publish-run` publishes only a candidate tied to the exact user-reviewed design digest, then creates a separate execution thread. `/harness publish` publishes without running, and `/harness run` offers published Harness IDs through autocomplete before reusing a selected version. Builder action buttons route to the same commands; publishing an already-published build resolves its recorded immutable version instead of failing.
- `/harness status`, `/harness list`, `/harness leave`, and `/harness cancel` inspect Builder/run state, list versions, remove only the current channel binding, or cancel a Builder without terminating its agent session. `/harness-help` and `/harness help` return the quick guide directly from the Gateway even when the channel has no linked agent session.
...
Harness v2 is available in Direct mode and expects the isolated bot/worker deployment. Its state is deliberately separate from channel/session state under `.connect/harnesses/state.json` by default. Drafts live under `drafts/<build-id>/`; published snapshots live under `published/<harness-id>/<version>-<digest>/`. Change the root with `CONNECT_HARNESS_ROOT`, or disable the feature with `CONNECT_HARNESS_ENABLED=false`. A Harness execution thread remains a normal Codex or Claude Code conversation surface: natural-language follow-ups steer an active turn, `/steer` is the explicit equivalent, `/interrupt` targets that thread's exact control key, `/queue` schedules a later turn, and `/fork` preserves the exact immutable Harness binding in the fork.

The Builder receives a read-only Codex sandbox or Claude Code plan mode with only read/search tools. The source channel receives the Builder thread link immediately, before agent provisioning finishes. Its opening message explains the four phases, natural-language answer patterns, revision behavior, and approval flow; action buttons cover recommended defaults, approval, publishing, running, and status. After each accepted Builder response, the Gateway posts a compact phase/turn/coverage/next-action indicator. Run progress distinguishes the main Agent from the Connector Worker tool executor and from actual delegated subagents. Timeline entries without meaningful detail and duplicated injected-prompt lifecycle events are omitted; plans, commands, edits, and checks retain their actor, purpose, and useful result. Every completed response must carry a hidden structured design brief; the Gateway validates and persists its phase, nine design sections, open questions, confirmation state, and canonical digest. Reserved Builder blocks are stripped again at the final Discord-delivery boundary so stale or missing Builder bindings cannot leak them. If the hidden brief or candidate is malformed or rejected, the Gateway asks the same Builder agent session to re-emit a corrected strict-JSON response up to three times without requiring another user message. Set `CONNECT_HARNESS_VALIDATION_RETRIES` from `0` through `5` to change that limit. A build cannot reach `review` before its third Builder response or `ready` before its fourth. `ready` requires a complete brief, no open questions, an explicit user confirmation, and byte-for-byte-equivalent design sections to the prior review. Candidate storage and publishing are gated on that reviewed digest, and any later design change invalidates the candidate.

The nine required sections cover purpose/triggers, real usage examples, inputs/context, workflow/decisions, outputs/success, constraints/permissions, resources/roles, failures/escalation, and normal plus edge validation cases. The Builder can emit only `SKILL.md`, `agents/*.md`, and passive `references/`, `tests/`, or `assets/` files within strict path/count/size limits. Hooks, MCP servers, background agents, permission overrides, traversal, symlinks, and unexpected snapshot files are rejected. Publishing is append-only per harness semantic version: the same digest is idempotent, while different content requires a new version.

At run time, the Worker—not the Discord Gateway—resolves the snapshot under the configured published root and verifies its canonical SHA-256 digest and every materialized file before starting the agent. Codex Harness runs require `CODEX_DISCORD_CODEX_RUNNER=app-server`; Claude Code loads the verified snapshot with `--plugin-dir`. A run stays pinned to its exact `harnessVersionId` on every turn and when `/fork` creates another execution session. `/compact` keeps its provider-native behavior without replacing the binding.

The requester is added to every Builder/run thread explicitly. When no explicit thread name is supplied, the Gateway fetches the original Discord thread name and derives `Original · 🧩 Harness Builder` or `Original · 🧰 <harness> v<version>`, clipping only the original-name prefix when Discord's 100-character limit requires it. A Harness run creates its progress card before waiting on the Worker, then updates it with the main Agent, Connector Worker tool executor, actual subagent, and Gateway roles, current owner and stage, latest Agent milestone, and command/edit/validation counters. Builder and Run timelines post meaningful Agent reports, analysis, plans, operation start/completion, file edits, validation, and session connection events. They suppress empty/raw lifecycle noise and injected-prompt echoes; command output is reduced to the intent, actual command, exit code, and duration. Discord-sized chunks are delivered in order without role mentions. Harness commands are durable requests: the run persists its request ID, Worker job ID, progress message ID, result message ID, session ID, and progress snapshot. After a Gateway restart, replay finds that request-bound run, reuses its existing thread and Worker job, resumes from the Worker's delivery cursor, and posts the durable result idempotently instead of provisioning another run.
...
In ordinary Codex and Claude Code threads, only agent-authored intermediate commentary is posted as new messages without role mentions. Command, file, search, reasoning-state, answer-state, and raw lifecycle events remain hidden there. Harness Builder and Run threads are the explicit verbose exception described above: they post the full event timeline. The final agent message is reserved for the final answer, duplicate commentary is filtered, and ordinary tasks retain their configured live-progress cap. Approval requests and final completion/failure notices retain the operator role mention. The pre-turn transcript sync advances its baseline without reposting prior conversation content.
