...
Unless an explicit name is supplied, Builder and execution thread names are derived from the original Discord thread, for example `Original task · 🧩 Harness Builder` and `Original task · 🧰 safe-review v1.0.0`. The requester is added to the execution thread automatically. The persistent progress card distinguishes the **Codex/Claude Code main agent** that owns the conversation and decisions, the **Connector Worker tool executor** that performs the commands, edits, and checks selected by that agent, an actual **subagent** only when delegation occurs, and the **Discord Gateway** that delivers progress. The timeline keeps meaningful agent reports, analysis, plans, commands, file changes, and validation with their actor and purpose while hiding empty lifecycle events and duplicate Harness prompt echoes. Commands show their purpose, actual command, and completion outcome instead of noisy output. If the Gateway restarts during a run, it reconnects to the same work using the persisted Discord request ID, Worker job ID, execution thread, and progress message ID instead of creating a duplicate run. Execution threads support the same natural-language chat, in-turn automatic steering, `/steer`, `/interrupt`, `/queue`, and `/fork` controls as ordinary agent threads while continuing to inject the exact Harness version.
